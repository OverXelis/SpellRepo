...
/**
 * Generates the spell name based on the naming rules.
 */
function makeSpellName(
  primaryRune: string,
  circleBase: string,
  modifiers: string[],
  control: string | null
  ...
  if (sortedMods.length === 0 && !control) {
    return `${primaryRune} - ${circleBase} - None`;
  }

  if (!control) {
    if (sortedMods.length === 1) {
      return `${primaryRune} - ${circleBase} - ${sortedMods[0]}`;
    }
    // Two modifiers
    return `${primaryRune} - ${circleBase} - ${sortedMods[0]} + ${sortedMods[1]}`;
  }

  if (sortedMods.length === 0) {
    // Control only
    return `${primaryRune} - ${circleBase} - ${control}`;
  }

  // One modifier + one control (modifier name first)
  return `${primaryRune} - ${circleBase} - ${sortedMods[0]} + ${control}`;
}
...
function createSpell(
  ...
  return {
    ...
    controlRune: control,
    spellName: makeSpellName(primaryRune, circleBase, modifiers, control),
    createdAt: Date.now(),
  };
}
